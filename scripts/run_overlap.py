...
from src.data.overlap import OverlapConfig, run_overlap_manifest
...
def load_env_file(path: Path) -> None:
    ...
    HF_TOKEN через secrets/env, а локально удобно держать его в .env.
    """
    if not path.exists():
        return
...
def parse_args() -> argparse.Namespace:
    """Описывает CLI-параметры overlap-stage."""
    parser = argparse.ArgumentParser(description="Estimate overlapped speech for audio manifests.")
    ...
    parser.add_argument("--model-name", default="pyannote/overlapped-speech-detection")
...
def main() -> None:
    # Локально токен удобно хранить в .env. На сервере переменная окружения обычно
    # приходит из secrets, и load_env_file не перетирает уже заданный HF_TOKEN.
    load_env_file(Path(".env"))
    args = parse_args()

    # CLI не содержит model/inference логики: она живет в src/data/overlap.py,
    # чтобы этот stage можно было тестировать без загрузки тяжелой pyannote-модели.
    ...
    # JSON-ответ нужен для простого мониторинга batch-запусков и сравнения CPU/GPU.
    print(
        json.dumps(
            {
                "status": "ok",
                "outputs": {
                    ...
                    "overlap_duration": round(outputs.overlap_duration, 3),
